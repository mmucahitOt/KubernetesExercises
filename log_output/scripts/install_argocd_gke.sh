...
ARGOCD_APP_PATH="${LOG_OUTPUT_ROOT}/argocd/applications/log-output-app.yaml"
...
if kubectl get application log-output-app -n argocd >/dev/null 2>&1; then
    print_success "ArgoCD Application created"
    
    print_info "Application status:"
    kubectl get application log-output-app -n argocd
    
    print_info "Waiting for initial sync (this may take a few minutes)..."
    print_info "You can check status with: kubectl get application log-output-app -n argocd -o yaml"
    
    SYNC_TIMEOUT=300
    ELAPSED=0
    while [ $ELAPSED -lt $SYNC_TIMEOUT ]; do
        SYNC_STATUS=$(kubectl get application log-output-app -n argocd -o jsonpath='{.status.sync.status}' 2>/dev/null || echo "Unknown")
        HEALTH_STATUS=$(kubectl get application log-output-app -n argocd -o jsonpath='{.status.health.status}' 2>/dev/null || echo "Unknown")
    ...
    if [ $ELAPSED -ge $SYNC_TIMEOUT ]; then
        ...
        print_info "Check status manually: kubectl get application log-output-app -n argocd"
    fi
else
...
print_info "Resources in exercises namespace:"
kubectl get all -n exercises || print_warning "No resources found in exercises namespace yet"
...
echo -e "  2. ${CYAN}Check application status${NC}: kubectl get application log-output-app -n argocd"
echo -e "  3. ${CYAN}View application details${NC}: kubectl describe application log-output-app -n argocd"
echo -e "  4. ${CYAN}Check deployed resources${NC}: kubectl get all -n exercises"
echo ""
