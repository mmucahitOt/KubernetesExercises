...
docker buildx build --platform linux/amd64 --no-cache -t log_output:${_IMAGE_TAG} "${LOG_OUTPUT_DIR}"
print_success "log_output image built"

print_info "Building read_output image..."
docker buildx build --platform linux/amd64 --no-cache -t read_output:${_IMAGE_TAG} "${READ_OUTPUT_DIR}"
print_success "read_output image built"

print_info "Building ping_pong image..."
docker buildx build --platform linux/amd64 --no-cache -t ping_pong:${_IMAGE_TAG} "${PING_PONG_DIR}"
print_success "ping_pong image built"

print_info "Building ping_pong_db image..."
docker buildx build --platform linux/amd64 --no-cache -t ping_pong_db:${_IMAGE_TAG} "${PING_PONG_DIR}/database"
