...
# This script is used to deploy a canary release of the todo_app_frontend application
...
TODO_APP_FRONTEND_ROOT="$(cd -- "${TODO_APP_ROOT}/../todo_app" >/dev/null 2>&1 && pwd -P)"
CANARY_RELEASE_DIR="${TODO_APP_ROOT}/manifests/canary_release_frontend"

# Build the Docker image
print_header "🐳 BUILDING DOCKER IMAGE"
print_info "Building todo_app image (this may take a few minutes)..."
# Use --load so the buildx image is loaded into the local Docker daemon for tagging/pushing.
docker buildx build --platform linux/amd64 --no-cache --load -t todo_app:${_IMAGE_VERSION} "${TODO_APP_FRONTEND_ROOT}"
...
docker tag todo_app:${_IMAGE_VERSION} $_DOCKER_REGISTRY/todo_app:${_IMAGE_VERSION}
docker push $_DOCKER_REGISTRY/todo_app:${_IMAGE_VERSION}
...
if ! kubectl get service todo-app-frontend-deployment-svc -n project >/dev/null 2>&1; then
    print_error "Stable service 'todo-app-frontend-deployment-svc' does not exist. Run kubernetes_deploy.sh first"
    exit 1
fi
...
if kubectl argo rollouts version >/dev/null 2>&1; then
    kubectl argo rollouts status rollout todo-app-frontend-rollout -n project --timeout=600s || {
    ...
    kubectl wait --for=condition=available rollout/todo-app-frontend-rollout -n project --timeout=600s || {
...
# Show final status
print_header "📊 STATUS"
kubectl get rollout todo-app-frontend-rollout -n project

print_header "🎉 DEPLOYMENT COMPLETE"
print_info "Commands:"
print_info "  Status: kubectl argo rollouts get rollout todo-app-frontend-rollout -n project"
print_info "  Promote: kubectl argo rollouts promote todo-app-frontend-rollout -n project"
print_info "  Abort: kubectl argo rollouts abort todo-app-frontend-rollout -n project"