...
# This script is used to deploy a canary release of the todo_app_backend application
...
TODO_APP_BACKEND_ROOT="$(cd -- "${TODO_APP_ROOT}/../todo_app_backend" >/dev/null 2>&1 && pwd -P)"
CANARY_RELEASE_DIR="${TODO_APP_ROOT}/manifests/canary_release_backend"

# Build the Docker image
print_header "🐳 BUILDING DOCKER IMAGE"
print_info "Building todo_app_backend image (this may take a few minutes)..."
# Use --load so the buildx image is loaded into the local Docker daemon for tagging/pushing.
docker buildx build --platform linux/amd64 --no-cache --load -t todo_app_backend:${_IMAGE_VERSION} "${TODO_APP_BACKEND_ROOT}"
...
docker tag todo_app_backend:${_IMAGE_VERSION} $_DOCKER_REGISTRY/todo_app_backend:${_IMAGE_VERSION}
docker push $_DOCKER_REGISTRY/todo_app_backend:${_IMAGE_VERSION}
...
export TODO_APP_BACKEND_DB_URL="postgres://todo_user:todo_password@todo-stset-db-svc:5432/todo_db"

# Check prerequisites
print_header "🔍 CHECKING PREREQUISITES"
if ! kubectl get namespace project >/dev/null 2>&1; then
    print_error "Namespace 'project' does not exist. Run kubernetes_deploy.sh first"
    exit 1
fi

if ! kubectl get service todo-app-backend-deployment-svc -n project >/dev/null 2>&1; then
    print_error "Stable service 'todo-app-backend-deployment-svc' does not exist. Run kubernetes_deploy.sh first"
    exit 1
fi

kubens project >/dev/null 2>&1
...
if kubectl argo rollouts version >/dev/null 2>&1; then
    kubectl argo rollouts status rollout todo-app-backend-rollout -n project --timeout=600s || {
    ...
    kubectl wait --for=condition=available rollout/todo-app-backend-rollout -n project --timeout=600s || {
...
# Show final status
print_header "📊 STATUS"
kubectl get rollout todo-app-backend-rollout -n project

print_header "🎉 DEPLOYMENT COMPLETE"
print_info "Commands:"
print_info "  Status: kubectl argo rollouts get rollout todo-app-backend-rollout -n project"
print_info "  Promote: kubectl argo rollouts promote todo-app-backend-rollout -n project"
print_info "  Abort: kubectl argo rollouts abort todo-app-backend-rollout -n project"